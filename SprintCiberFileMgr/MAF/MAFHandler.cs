...
namespace TruMobility.Network.Services
{
    public class MAFHandler
    {
        ...
        private string m_connection  = ConfigurationManager.AppSettings["SQLConnectString"];
        ...
        private MAFFileReader m_rdr = null;
        ...
        private void ProcessFileWatcher(object sender, FileSystemEventArgs e)
        {
            try
            {
                ...
                {
                    m_logger.WriteToLogFile("ProcessFileWatcher()::Enqueue'ngTheFile");
                ...
                while (true)
                {
                    ...
                    try
                    {
                        if (m_fileNameQ.Count > 0)
                        {
                            ...
                            lock (m_fileNameQ.SyncRoot)
                            {
                                m_logger.WriteToLogFile("-NFORMATIONAL::MAFHandler::ProcessJobControlFileThread():DequeuingTheFileToProcess");
                        ...
                        if (fName != String.Empty)
                        {
                            ...
                            m_dbMgr.UpdateFileStatus( f );
                        }

                    }//try
                    catch (System.Threading.ThreadAbortException te)
                    {
                        m_logger.WriteToLogFile("-NEXCEPTIONCAUGHT::MAFHandler::ProcessJobControlFileThread():ExceptionCaught:ThreadAbortExceptionCaught" + te.Message + te.StackTrace);
                        return;
                    }
                    catch (System.Exception ex)
                    {// generic system exception

                        m_logger.WriteToLogFile("-NEXCEPTIONCAUGHT::MAFHandler::ProcessJobControlFileThread():ExceptionCaught:SystemExceptionCaught" + ex.Message + ex.StackTrace);
        ...
        private void MoveTheFile(string fileName)
        {
            ...
            try
            {
                ...
                m_logger.WriteToLogFile(DateTime.Now.ToShortDateString() + ":" + DateTime.Now.ToLongTimeString() + "-NFORMATIONAL:moved file: " + fileName + " to location: " + m_filemovePath + newFileName + "\r\n");
            }
