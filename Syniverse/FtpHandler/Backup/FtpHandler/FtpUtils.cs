...
namespace EPCS.Syniverse.TN
{
    public class FtpUtils
    {
        ...
        public static List<string> ListDirectoryOnFtpSite(Uri ftpUri, string user, string pass)
        {
            ...
            {
                m_logger.WriteToLogFile("ECaught:" + se.Message + se.StackTrace);
        ...
        /// <summary>
        /// method to post a file to the specified FTP site
        /// </summary>
        /// <param name="ftpUri"></param>
        /// <param name="user"></param>
        /// <param name="pass"></param>
        /// <returns></returns>
        public static byte[] PutFileToSite(Uri ftpUri, string fileName, string user, string pass)
        {
            byte[] response = null;
            ...
            try
            {
                response = request.UploadFile(ftpUri.ToString(), fileName);
            }
            catch (WebException e)
            {
                m_logger.WriteToLogFile("FtpUtils::GetFileFromSite();ECaught: " + e.Message);
